...
BOOL CSystemTray::ShowBallonTip(wchar_t* szTitle, wchar_t* szMsg, int nTimeOut)
{
	VERIFY(nTimeOut*1000  >= 10); // in Windows 2000 nTimeout is 10 seconds, If even set less no point, so a check just in case.

	ASSERT(wcscmp(szTitle, _T("")) != 0); // Sure that balloon title == "" ??
	ASSERT(wcscmp(szMsg, _T("")) != 0); // Sure that balloon message == "" ?

	if( (wcslen(szTitle) < 1) || (wcslen(szMsg) < 1))
		return FALSE;
	...
	m_tnd.dwInfoFlags      = NIIF_INFO; // add an icon to a balloon ToolTip
			// Flag Description 
	...
	int len = wcslen(szMsg);
	if(len > 255)
		len = 255;
	_tcsncpy(m_tnd.szInfo, szMsg, len);
	m_tnd.szInfo[len] = 0;//null termination

	len = wcslen(szTitle);
	if(len > 63)
		len = 63;

	_tcsncpy(m_tnd.szInfoTitle, szTitle, len);
	m_tnd.szInfoTitle[len] = 0;//null termination

	m_tnd.uTimeout         = nTimeOut * 1000;	    
